...
class TestGoalProgressUsingPatient(PlansMixin, APITestCase):
    """
    Test cases for :model:`plans.GoalProgress` using a patient
    as the logged in user.
    """
    ...
    def test_get_goal_progress_detail_not_owner(self):
        goal_progress = self.create_goal_progress()
        url = reverse(
            'goal_progresses-detail',
            kwargs={'pk': goal_progress.id}
    ...
    def test_full_update_goal_progress_not_owner(self):
        payload = {
            'goal': self.goal.id,
            'rating': random.randint(1, 5),
        }
    ...
    def test_partial_update_goal_progress_not_owner(self):
        payload = {
            'rating': random.randint(1, 5),
        }
        goal_progress = self.create_goal_progress()
