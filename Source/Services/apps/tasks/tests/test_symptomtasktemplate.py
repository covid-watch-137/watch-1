...
class TestSymptomTaskTemmplateUsingEmployee(TasksMixin, APITestCase):
    ...
    def test_update_start_on_day_symptomtasktemplate_with_ongoing_plans(self):
        template = self.create_care_plan_template(duration_weeks=6)
        start_on_day = 2
        ...
        task_template = self.create_symptom_task_template(
            plan_template=template,
            start_on_day=4,
        )
        ...
        tasks = task_template.symptom_tasks.filter(due_datetime__gte=now)
        for task in tasks:
            self.assertEqual(task.due_datetime.date(), due_datetime.date())

    def test_update_frequency_symptomtasktemplate_with_ongoing_plans(self):
        total_duration = 6
        past_duration = 2
        plans_count = 3
        template = self.create_care_plan_template(
    ...
    def test_update_repeat_amount_symptomtasktemplate_with_ongoing_plans(self):
        total_duration = 6
        past_duration = 2
        plans_count = 3
        repeat_amount = 5
    ...
    def test_update_appear_time_symptomtasktemplate_with_ongoing_plans(self):
        template = self.create_care_plan_template(duration_weeks=6)
    ...
    def test_update_due_time_symptomtasktemplate_with_ongoing_plans(self):
        template = self.create_care_plan_template(duration_weeks=6)
